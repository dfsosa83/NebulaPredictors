...
model1 = pickle.load(open(working_dir + "/models/trained_models_trial_3/xg_model_signed_target_Q1.sav","rb"))
model2 = pickle.load(open(working_dir + "/models/trained_models_trial_3/cb_model_signed_target_Q2.sav", 'rb'))
model3 = pickle.load(open(working_dir + "/models/trained_models_trial_3/lg_model_signed_target_Q3.sav","rb"))
model4 = pickle.load(open(working_dir + "/models/trained_models_trial_3/hist_model_signed_target_Q4.sav","rb"))
#p95
model5 = pickle.load(open(working_dir + "/models/trained_models_trial_3/lg95_model_signed_target.sav","rb"))
model6 = pickle.load(open(working_dir + "/models/trained_models_trial_3/hist_model95_signed_target.sav","rb"))

# Define a custom MLPRegressor subclass
class CustomMLPRegressor(MLPRegressor):
    def __init__(self, random_seed=None, **kwargs):
        if random_seed is not None:
            np.random.seed(random_seed)
        super().__init__(**kwargs)
# Load the trained meta_model using the CustomMLPRegressor
model_path = working_dir + "/models/trained_models_trial_3/meta_6_models_p95_signed_MLP.sav"
import joblib
meta_model = joblib.load(model_path)
#meta_model = pickle.load(open(working_dir + "/models/trained_models_trial_3/meta_6_models_p95_signed_MLP.sav","rb"))
